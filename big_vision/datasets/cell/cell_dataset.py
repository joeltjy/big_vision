...
class CellDataSource(ds_core.DataSource):
    ...
    def get_tfdata(self, ordered=False, *, process_split=True, allow_cache=True, **kw):
        return (self._cached_get_dataset if allow_cache else self._get_dataset)(ordered, process_split, **kw)
    
    def _count_examples(self):
        """Count total examples in the generator dataset."""
        count = 0
        for _ in self.generator_fn(self.dataset, self.lookup_dataset):
            count += 1
        return count
...
if __name__ == "__main__":
    # Create a dataset
    dataset = CellDataSource(split="train", data_dir=DATA_DIR, lookup_dir=LOOKUP_DIR)
    print(dataset.total_examples)
    print(dataset.num_examples_per_process)